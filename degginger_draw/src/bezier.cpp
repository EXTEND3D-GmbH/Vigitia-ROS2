...
double PerpendicularDistance( const cv::Point2d& pt,
...
vector< cv::Point2d > RamerDouglasPeucker( const vector< cv::Point2d >& pointList, double epsilon )
{
	vector< cv::Point2d > out;

	if ( pointList.size() < 2 ) throw invalid_argument( "Not enough points to simplify" );
	...
	if ( dmax > epsilon ) {
		// Recursive call
		vector< cv::Point2d > firstLine( pointList.begin(), pointList.begin() + index + 1 );
		vector< cv::Point2d > lastLine( pointList.begin() + index, pointList.end() );
		vector< cv::Point2d > recResults1 = RamerDouglasPeucker( firstLine, epsilon );
		vector< cv::Point2d > recResults2 = RamerDouglasPeucker( lastLine, epsilon );

		// Build the result list
		out.assign( recResults1.begin(), recResults1.end() - 1 );
		out.insert( out.end(), recResults2.begin(), recResults2.end() );
		if ( out.size() < 2 ) throw runtime_error( "Problem assembling output" );
	} else {
 ...
 */
void FitCubic( vector< cv::Point2d > const& d,
			   vector< BezierCurve >* bezCurves,
			   int first,
			   int last,
			   cv::Vec2d tHat1,
			   cv::Vec2d tHat2,
...
void generateBezier( vector< cv::Point2d > const& d,
					 vector< cv::Vec2d >* bezCurve,
					 int first,
					 int last,
					 vector< double > const& uPrime,
					 cv::Vec2d tHat1,
					 cv::Vec2d tHat2 )
{
...
vector< double >* reparameterize( vector< cv::Point2d > const& d,
								  int first,
								  int last,
...
double computeMaxError( vector< cv::Point2d > const& d,
						int first,
						int last,
						vector< cv::Vec2d >* bezCurve,
						vector< double > const& u,
